...
# Update de paginatitel voor versie v52
st.set_page_config(page_title="Factuurcontrole Tool (Trevian) v52 – auto-import (no-fuzzy, lichte historie)", layout="wide")
...
with col2:
    ...
    # Werk de versieaanduiding bij naar v52
    st.markdown('<p style="color:#888;">Onderdeel van Trevian Finance & Control — v52 (auto-import)</p>', unsafe_allow_html=True)
...
def process_pdf_path(path: str, prijzenboek, prijs_codes_norm, aggregeer_per_taakcode=True, TOLERANTIE=0.05, use_fuzzy=True, fuzzy_threshold=92):
    ...
    rows = []
    # Houd een set bij van regels die al verwerkt zijn (bijvoorbeeld als onderdeel van een taakcode of onbekende code)
    ...
    for found_code, matched_code in code_map.items():
        ...
        # Markeer alle regels die bij deze taakcode horen als gebruikt, zodat we ze later niet nog eens toevoegen
        for rr in relevante_regels:
            used_lines.add(rr)
    ...
    if unmatched_codes:
        ...
        skip_keywords = [
            ...
            # Specifieke teksten die geen factuurregel zijn maar wel cijfers bevatten
            "arbeidsdeel", "postbus", "btw nummer", "btw nr", "btwnr",
            # Totale en btw regels
            "totaal", "subtotaal", "btw verlegd",
            # BTW verkoop hoog/laag regels uit eindtotaal (bijv. "BTW verkoop hoog (21% van € 6.557,75)")
            "btw verkoop", "verkoop hoog", "verkoop laag",
        ]
        # Voeg context-specifieke skipwoorden toe om BTW/totaal-regels die over meerdere regels lopen te negeren.
        # Sommige facturen splitsen regels als "BTW verkoop hoog (21% van €" en de regel erna "30.459,69) 0,00 €". De prijsregel
        # bevat dan geen sleutelwoord maar hoort niet als taakregel meegenomen te worden. We kijken daarom ook naar
        # aangrenzende regels bij het bepalen of we moeten overslaan.
        context_skip_keywords = [
            "btw", "btw verlegd", "btw verkoop", "verkoop hoog", "verkoop laag",
            "totaal", "subtotaal", "loonkosten", "loonkostenbestanddeel", "overmaken",
            "g-rekening", "restant", "loon"
        ]
        for uc in unmatched_codes:
            ...
            for regel in relevante_regels:
                if not regel:
                    continue
                low = (regel or "").lower()
                # Bepaal of we deze regel moeten overslaan
                skip_line = False
                # 1) Direct overslaan op basis van skip_keywords (administratieve termen)
                if any(kw in low for kw in skip_keywords):
                    skip_line = True
                else:
                    # 2) Kijk naar aangrenzende regels om opgesplitste BTW/totaal-regels te detecteren
                    try:
                        idx = regels_gevonden.index(regel)
                    except Exception:
                        idx = -1
                    if idx != -1:
                        # Controleer de vorige regel
                        if idx > 0:
                            prev_low = (regels_gevonden[idx - 1] or "").lower()
                            if any(kw in prev_low for kw in context_skip_keywords):
                                skip_line = True
                        # Controleer de volgende regel alleen als we nog niet hebben besloten om over te slaan
                        if not skip_line and idx + 1 < len(regels_gevonden):
                            next_low = (regels_gevonden[idx + 1] or "").lower()
                            if any(kw in next_low for kw in context_skip_keywords):
                                skip_line = True
                if skip_line:
                    used_lines.add(regel)
                    continue
                bedragen = extract_bedragen(regel)
                ...
                if prijs_val is None and not qty_candidates:
                    # markeer als gebruikt zodat deze regel niet in de aanvullende ronde wordt meegenomen
                    used_lines.add(regel)
                    continue
                rows.append({
                    "Bestandsnaam": os.path.basename(path),
                ...
                # markeer deze regel als gebruikt, omdat hij nu is verwerkt
                used_lines.add(regel)
    # === Verwerk regels zonder (herkende) taakcode ===
    # Na het verwerken van gematchte en ongematchte codes kijken we of er nog regels over zijn die bedragen bevatten,
    # maar waarvoor geen taakcode is gevonden. Deze regels moeten ook in de output terechtkomen als "Onbekende taakcode".
    # Hierbij gebruiken we dezelfde skip_keywords om irrelevante regels uit te filteren.
    # Definieer dezelfde skip_keywords voor gebruik buiten de unmatched_codes-block
    # Onderstaande sleutelwoorden worden gebruikt om regels zonder taakcode te filteren. Directe skip
    # betekent dat we regels met deze woorden in dezelfde regel overslaan. Voor context skip wordt een
    # tweede lijst gebruikt: als een aangrenzende regel een van deze woorden bevat, slaan we de huidige regel
    # ook over. Dit helpt om opgesplitste BTW/totaal-regels te negeren.
    _skip_keywords_no_code = [
        "iban", "banknummer", "rabo", "rabobank", "rekening",
        "datum", "factuurnummer", "factuurnr", "werkadres", "opdrachtnr", "opdracht", "uw nummer",
        "bij betaling", "betalingskenmerk", "betaal", "betaaldatum", "uiterste",
        # we willen bepaalde administratie-regels zoals 'werkorder' alleen overslaan als ze expliciet voorkomen op dezelfde regel,
        # maar niet via context; deze blijven hier staan
        "werkorder", "g-rekening",
        # Regels die geen factuurlijnen zijn maar wel getallen bevatten
        "arbeidsdeel", "postbus", "btw nummer", "btw nr", "btwnr",
    ]
    _context_skip_no_code = [
        "btw", "btw verlegd", "btw verkoop", "verkoop hoog", "verkoop laag",
        "totaal", "subtotaal", "loonkosten", "loonkostenbestanddeel", "overmaken", "g-rekening", "restant", "loon"
    ]
    for i, regel in enumerate(regels_gevonden):
        # Sla over als deze regel al eerder verwerkt is (bij een match of onbekende code)
        if regel in used_lines:
            continue
        if not regel:
            continue
        low = (regel or "").lower()
        skip_line = False
        # 1) Skip op basis van directe keywords
        if any(kw in low for kw in _skip_keywords_no_code):
            skip_line = True
        else:
            # 2) Contextuele skip: kijk naar aangrenzende regels voor BTW/totaal-context
            if i > 0:
                prev_low = (regels_gevonden[i - 1] or "").lower()
                if any(kw in prev_low for kw in _context_skip_no_code):
                    skip_line = True
            if not skip_line and i + 1 < len(regels_gevonden):
                next_low = (regels_gevonden[i + 1] or "").lower()
                if any(kw in next_low for kw in _context_skip_no_code):
                    skip_line = True
        if skip_line:
            used_lines.add(regel)
            continue
        bedragen = extract_bedragen(regel)
        qty_candidates = extract_qty_candidates(regel)
        # Voeg alleen regels toe met een bedrag of hoeveelheid
        if not bedragen and not qty_candidates:
            used_lines.add(regel)
            continue
        try:
            aantal_unknown = float(qty_candidates[0]) if qty_candidates else 1.0
